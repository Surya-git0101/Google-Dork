...
def dorks():
    ...
    try:
        ...
        if user_choice == "all":
            total_results = 1000  # Google's maximum practical limit
        else:
            try:
        ...
        # Use the correct parameters for your library version
        for result in search(
            dork, 
            num_results=total_results,  # Number of results to fetch
            sleep_interval=2            # Delay between requests (prevents blocking)
        ):
        ...
        print(f"\n{Colors.RED}[!] User Interruption Detected!{Colors.RESET}\n")
        sys.exit(1)
    except Exception as e:
        print(f"{Colors.RED}[ERROR] {str(e)}{Colors.RESET}")
    
    print(f"{Colors.GREEN}\n[✔] Automation Complete!{Colors.RESET}")
    sys.exit()
